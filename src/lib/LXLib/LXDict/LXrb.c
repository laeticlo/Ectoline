...
#define mk_new_ext(new, kkkey, vvval) {\
  new = (Rb_node) LXMalloc(sizeof(struct rb_node));\
  new->v.val = vvval;\
  new->k.key = kkkey;\
  setext(new);\
  setblack(new);\
...
static void mk_new_int(Rb_node l, Rb_node r, Rb_node p, int il)
{
  ...
  newnode = (Rb_node) LXMalloc(sizeof(struct rb_node));
  setint(newnode);
  setred(newnode);
  setnormal(newnode);
...
{
  static char sEmpty[2] = "";
  Rb_node head;
 
  head = (Rb_node) LXMalloc (sizeof(struct rb_node));
...
Rb_node rb_find_key_n(Rb_node n, char *key, int *fnd)
{
  ...
  *fnd = 0;
  if (!ishead(n)) {
    fprintf(stderr, "rb_find_key_n called on non-head 0x%lx\n", (unsigned long) n);
    exit(1);
  }
...
Rb_node rb_find_ikey_n(Rb_node n, int ikey, int *fnd)
{
  *fnd = 0;
  if (!ishead(n)) {
    fprintf(stderr, "rb_find_ikey_n called on non-head 0x%lx\n", (unsigned long) n);
    exit(1);
  }
  if (n->p.root == n) return n;
...
void rb_delete_node(Rb_node n)
{
  ...
  if (ishead(n)) {
    fprintf(stderr, "Cannot delete the head of an rb_tree: 0x%lx\n", (unsigned long) n);
    exit(1);
  }
...
void rb_print_tree(Rb_node t, int level)
{
  ...
  if (ishead(t) && t->p.parent == t) {
    ...
    printf("Head: 0x%lx.  Root = 0x%lx\n", (unsigned long) t, (unsigned long) t->p.root);
    rb_print_tree(t->p.root, 0);
  } else {
    if (isext(t)) {
      for (i = 0; i < level; i++) putchar(' ');
      printf("Ext node 0x%lx: %c,%c: p=0x%lx, k=%s\n", 
              (unsigned long) t, isred(t)?'R':'B', isleft(t)?'l':'r',
              (unsigned long) t->p.parent, t->k.key);
    } else {
      ...
      printf("Int node 0x%lx: %c,%c: l=0x%lx, r=0x%lx, p=0x%lx, lr=(%s,%s)\n", 
              (unsigned long) t, isred(t)?'R':'B', isleft(t)?'l':'r', 
              (unsigned long) t->c.child.left, 
              (unsigned long) t->c.child.right, 
              (unsigned long) t->p.parent, 
              t->k.lext->k.key, t->v.rext->k.key);
    }
  }
}
...
void rb_iprint_tree(Rb_node t, int level)
{
  ...
  if (ishead(t) && t->p.parent == t) {
    ...
    printf("Head: 0x%lx.  Root = 0x%lx, < = 0x%lx, > = 0x%lx\n", 
            (unsigned long) t, (unsigned long) t->p.root,
            (unsigned long) t->c.list.blink,
            (unsigned long) t->c.list.flink);
    ...
    if (isext(t)) {
      ...
      printf("Ext node 0x%lx: %c,%c: p=0x%lx, <=0x%lx, >=0x%lx k=%d\n", 
              ...
              (unsigned long) t->p.parent, 
              (unsigned long) t->c.list.blink,
              (unsigned long) t->c.list.flink, t->k.ikey);
    } else {
      ...
      printf("Int node 0x%lx: %c,%c: l=0x%lx, r=0x%lx, p=0x%lx, lr=(%d,%d)\n", 
              ...
              (unsigned long) t->c.child.right, 
              (unsigned long) t->p.parent,
              t->k.lext->k.ikey, t->v.rext->k.ikey);
    }
  }
}
...
int rb_nblack(Rb_node n)
{
  int nb;
  if (ishead(n) || isint(n)) {
    fprintf(stderr, "ERROR: rb_nblack called on a non-external node 0x%lx\n",
            (unsigned long) n);
    exit(1);
  }
  nb = 0;
...
int rb_plength(Rb_node n)
{
  int pl;
  if (ishead(n) || isint(n)) {
    fprintf(stderr, "ERROR: rb_plength called on a non-external node 0x%lx\n",
            (unsigned long) n);
    exit(1);
  }
  pl = 0;
...
{
  return rb_insert_b(rb_find_ikey(tree, ikey), (char *) (intptr_t) ikey, val);
}
