...
/* ---------------------------------------------------------------- */
/*                                                                  */
/* @notes: original file from Minix (c)                             */
/*                                                                  */
/* @history:                                                        */
...
#define CFAIL(m)        { LX_ERROR(m, 0); return((char *)NULL); }
#define RFAIL(m)        { LX_ERROR(m, 0); return((LXRegExp *)NULL); }
...
char *exp;
{
  ...
  regcode = &regdummy;
  regc(MAGIC);
  if (reg(0, &flags) == (char *)NULL) return((LXRegExp *)NULL);
  ...
  /* Allocate space. */
  r = (LXRegExp *) LXMalloc(sizeof(LXRegExp) + (unsigned) regsize);
  if (r == (LXRegExp *)NULL) RFAIL("out of space");

  /* <Gloup> added */
  (void) memset(r, 0, sizeof(LXRegExp));
...
/* Forwards.
 */
static int regtry(LXRegExp *prog, char *string);
 ...
 */
int LXRegExec(prog, string, bolflag)
register LXRegExp *prog;
...
Bool bolflag;
{
  ...
  /* Be paranoid... */
  if (prog == (LXRegExp *)NULL || string == (char *)NULL) {
        LX_ERROR("NULL parameter", 0);
        return(0);
  }

  /* Check validity of program. */
  if (UCHARAT(prog->program) != MAGIC) {
        LX_ERROR("corrupted program", 0);
        return(0);
  }
...
char *op;
{
  ...
  switch (OP(op)) {
      ...
      default:  LX_ERROR("corrupted opcode", 0); p = (char *) NULL; break;
  }
